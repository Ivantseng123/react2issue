...
| Q8 | **Slack mrkdwn 危險字元 `<>&` 不在 config 擋，render 時 `slackEscape` 自動轉義** | 運維可能把 `<@U123>` 貼進 pool（會變 ping 事故）；config validation 的職責是「語意錯」，Slack 特殊字元屬「顯示安全」該由 render 層負責。 |
| Q9 | **Config validation fail-fast：空白/長度錯 → startup error** | Pre-launch 階段 fail-fast 成本低，比 silent-skip / auto-truncate 透明；運維 deploy 時看到明確錯誤比事後 debug 好。 |
| Q10 | **`strings.TrimSpace` 正規化 at config load**，rune 長度算 trim 後字串 | 手滑多打空格是運維常見操作；trim 是字串衛生，非「auto-fix 錯誤」，跟 Q9 不衝突。純空白 → trim 後空 → fail。 |
| Q11 | **自寫 `slackEscape` 只轉 `<>&`**，不用 `html.EscapeString` | `html.EscapeString` 還會轉 `'"` 成 `&#39;`/`&#34;`，Slack mrkdwn 對數字實體沒官方保證，可能顯示成字面 `&#39;`。三字元夠。 |
| Q12 | **Escape 範圍涵蓋 `label` + `r.AgentCmd`** | AgentCmd 來自 agent CLI command，理論上也可能被運維塞進特殊字元；多 escape 一次零成本，一致性比微優化重要。 |
...
worker/config/validate.go      +TrimSpace + 空白/rune 長度驗證
worker/config/defaults.go      +NicknamePool: nil（yaml.Marshal 輸出 []，empirically verified）
...
worker/worker.go               +呼叫 pickNicknames，傳進 pool.Config；池<count 時 warn log
shared/queue/interface.go      +WorkerInfo.Nickname、StatusReport.WorkerNickname
shared/queue/redis_jobqueue.go （無動；JSON marshal 自動帶 Nickname，omitempty 兼容舊 JSON）
app/bot/status_listener.go     +formatWorkerLabel(workerID, nickname) + slackEscape()
...
**`slackEscape`**（`app/bot/status_listener.go`，新增）：

```go
// slackEscape escapes the three characters Slack mrkdwn treats as legacy XML
// entities. Ordered: '&' first so '<' / '>' replacements don't get double-escaped.
...
不用 `html.EscapeString`——那會額外轉 `'`/`"` 成數字實體，Slack mrkdwn 對 `&#39;` 的 decode 行為沒官方保證，可能直接顯示成字面 `&#39;`。
...
1. **先 normalize**：`trimmed := strings.TrimSpace(entry)`。前後空白丟棄，後續以 trim 後字串為準（也會覆寫原 slice 位置，存乾淨版）。
2. **trim 後空字串 → error**：`nickname_pool[i] is empty or whitespace`。
3. **`utf8.RuneCountInString(trimmed) > 32` → error**：`nickname_pool[i] length N exceeds 32 runes`。
4. **任意 Unicode 合法**（無字元白名單；emoji、CJK、拉丁字母全 OK）。
5. **允許重複條目**（不去重、不報錯）。
6. **`<>&` 字元 config 層不擋**——由 render 時 `slackEscape` 處理（見下方 `renderStatusMessage`）。

失敗模式：任一條目錯，`Validate(cfg)` 把所有錯誤累積後 return，worker process 在啟動時 fail，stderr 印出每個 index 為什麼錯（跟既有 `Validate` 一致）。

不做的驗證：
- `len(pool)` 跟 `count` 的關係（獨立，池不夠時在 `worker.Run` 發 warn log）。
- 跨條目唯一性（重複本案視為合法）。
- Slack mrkdwn 特殊字元（屬 render 層責任）。
...
- `worker/config/validate_test.go`：
  - 空池合法、空字串條目 fail、純空白條目 fail、超長（33 rune）fail、剛好 32 rune pass、Unicode / emoji 條目 pass。
  - 前後空白 trim 後儲存乾淨版（`"  小明  "` → 驗證後條目變 `"小明"`）。
  - 重複條目合法（不報錯）。
  - 含 `<` / `>` / `&` 的條目 **合法**（config 層不擋，交 render 層 escape）。
...
- `app/bot/status_listener_test.go`：
  ...
  - **escape 測試**：`WorkerNickname = "<@U123>"` → 渲染含 `&lt;@U123&gt;`，不含原始 `<`；`AgentCmd = "foo&bar"` → 渲染含 `foo&amp;bar`。
