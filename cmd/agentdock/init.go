...
// runInitWorker writes a starter worker.yaml. Built-in agents are NOT
// snapshotted; mergeBuiltinAgents fills them from the current binary at
// startup, so operators always pick up the latest args without re-running init.
...
func promptWorkerInit(cfg *workerconfig.Config) error {
	...
	fmt.Fprintln(prompt.Stderr)
	names := make([]string, 0, len(workerconfig.BuiltinAgents))
